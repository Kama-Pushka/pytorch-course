...
### Аугментации (получаем новые данные путем изменения старых)
...
class Posterize:
    ...
    def __call__(self, img):
        img_np = img.numpy()
        factor = 2 ** (8 - self.bits) # шаг квантования

        img_np = (img_np * 255).astype(np.uint8)
        imp_np = (img_np // factor) * factor
        return torch.from_numpy(imp_np.astype(np.float32) / 255.0)
...
class ElasticTransform:
    ...
    def __call__(self, img):
        ...
        img_deformed = cv2.remap(img_np, x.astype(np.float32), y.astype(np.float32), cv2.INTER_LINEAR)
